...
from services.instagram import (
    ...
    send_regular_buttons_to_comment,
...
def _send_main_message_to_comment(comment_id: str, config: dict):
    ...
    if buttons:
        return send_regular_buttons_to_comment(
            comment_id=comment_id,
            text=text or "Here is your access",
            buttons=buttons[:3],
        )
    ...
    return send_dm_with_button(
        ...
        text=text or "Here is your access",
        button_label="Open",
        button_url="https://instagram.com/",
    ) if False else send_dm_with_postback_button(
        comment_id=comment_id,
        text=text or "Here is your access",
        button_label="Done",
        payload_value="ACCESS_SENT",
    )
...
async def handle_webhook(request: Request):
    ...
    for entry in body.get("entry", []):
        for change in entry.get("changes", []):
            ...
            # If follow gate is required and user already follows,
            # send final access directly from comment private reply.
