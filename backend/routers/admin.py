...
class AccessDeliveryConfig(BaseModel):
    message: str = ""
    button_label: str = ""
    access_link: str = ""
    secondary_text: str = ""
...
class ReelConfigUpdate(BaseModel):
    ...
    comment_reply: str = "Sent you a DM!"
    first_dm: FirstDMConfig
    follow_gate: FollowGateConfig
    access_delivery: AccessDeliveryConfig
...
async def update_reel(media_id: str, config: ReelConfigUpdate):
    try:
        updated = update_reel_config(media_id, config.dict())
