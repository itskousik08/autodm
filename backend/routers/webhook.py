...
def _send_main_message(igsid: str, config: dict):
    ...
    if not text and not buttons:
        return {"error": {"message": "Main message is empty"}}
    ...
    return {"error": {"message": "Main message needs at least one button"}}
...
async def handle_webhook(request: Request):
    ...
    for entry in body.get("entry", []):
        for change in entry.get("changes", []):
            ...
            igsid = value.get("from", {}).get("id") or ""
